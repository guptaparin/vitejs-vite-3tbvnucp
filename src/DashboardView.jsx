...
export default function DashboardView({ data, onReset }) {
  ...
  let baseScore = (cgpaNum * 6) + (hoursNum * 3) + (projectCount * 5) + (skillCount * 2);
    ...
    <div className="max-w-5xl mx-auto space-y-8 p-2 sm:p-4 animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-zinc-900 border border-zinc-800 rounded-2xl p-6 flex flex-col justify-between">
          ...
          {/* Conditional Status Warnings Box */}
          <div className="mt-6 pt-6 border-t border-zinc-800 space-y-4">
            {isBurnout && (
              <div className="flex gap-3 bg-red-500/10 border border-red-500/20 rounded-xl p-4 text-red-400">
                <Flame className="h-5 w-5 flex-shrink-0 mt-0.5" />
                <div>
                  <h4 className="font-semibold text-sm text-white">Burnout Warning Triggered</h4>
                  <p className="text-xs text-zinc-400 mt-1">Your high study hours and high CGPA put you at high risk for quick exhaustion. Remember to space out your tasks.</p>
                </div>
              </div>
            )}

            {isDelusional && (
              <div className="flex gap-3 bg-amber-500/10 border border-amber-500/20 rounded-xl p-4 text-amber-400">
                <AlertTriangle className="h-5 w-5 flex-shrink-0 mt-0.5" />
                <div>
                  <h4 className="font-semibold text-sm text-white">Alignment Gap Detected</h4>
                  <p className="text-xs text-zinc-400 mt-1">Your preparation schedule is light compared to your target goal metrics. Increasing active build projects will boost this rating.</p>
                </div>
              </div>
            )}

            {!isBurnout && !isDelusional && (
              <div className="flex gap-3 bg-zinc-950 border border-zinc-850 rounded-xl p-4 text-zinc-400">
                <ShieldCheck className="h-5 w-5 text-emerald-500 flex-shrink-0 mt-0.5" />
                <div>
                  <h4 className="font-semibold text-sm text-white">Stable Progression</h4>
                  <p className="text-xs text-zinc-500 mt-1">Your input pace looks balanced. Keep executing against your specific roadmap targets below.</p>
                </div>
              </div>
            )}
          </div>
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6">
          ...
          <ul className="space-y-3">
            {currentRole.tasks.map((task, index) => (
              <li key={index} className="text-sm text-zinc-300 flex items-start gap-2 bg-zinc-950 p-3 rounded-xl border border-zinc-800">
